echo "Creating bl backend vm"
docker-machine create --driver virtualbox bl-backend
eval $(docker-machine env bl-backend)
echo "starting exis node containers"
# docker build -t exis-node ./node
# docker run -d -e --name=exis-node-01 -t exis-node 
# docker run -d -e --name=exis-node-02 -t exis-node 
# docker run -d -e --name=exis-node-03 -t exis-node
